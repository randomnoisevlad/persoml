...
def load_train_data(images_paths, masks_paths):
    ...
    for i in range(len(images_paths)):
        train_images[i] = cv2.imread(images_paths[i])
        train_masks[i] = cv2.imread(masks_paths[i], cv2.IMREAD_UNCHANGED)[..., None] > 0
